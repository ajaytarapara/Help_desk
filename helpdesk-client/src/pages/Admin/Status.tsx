...
const Status = () => {
  ...
  const [sortKey, setSortKey] = useState<string>("statusId");
  ...
  const getStatusList = async () => {
    ...
    const response = await dispatch(getAllStatusThunk(payload));

    const paginationData = (response.payload as PaginationResponse<any>)?.data;

    if (paginationData) {
      setStatusList(paginationData.items);
      setTotalRow(paginationData.totalCount);
    }
  };

  const handleDelete = async () => {
    await dispatch(deleteStatusThunk(deletedId));
    setPage(DefaultPageNumber);
    getStatusList();
    setDialogOpen(false);
  };
      ...
      <Container maxWidth="xl">
        <TicketHeader>
          ...
          <CustomButton
            variant="contained"
            startIcon={<Add />}
            onClick={() => navigate(Routes.ADMIN_STATUS_CREATE)}>
            Create Status
          </CustomButton>
        </TicketHeader>
        ...
        <DeleteDialog
          ...
          title="Delete Status"
          message="Do you want to delete this status? This action cannot be undone."
        />
      </Container>
    </TicketRoot>
  );
